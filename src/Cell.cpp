...
namespace Netlist {
 ...
 void  Cell::toXml ( ostream& stream ) const
    ...
    while ( true ) {
      ...
      switch ( state ) {
        ...
        case EndNets:
          ...
          break;
        case BeginSymbol:  //On affiche toues les symbol
  ...
  {
    //on va lire un fichier xml de nom name
    string           cellFile = "../cells/" + cellName + ".xml";
    xmlTextReaderPtr reader; // on initialise notre reader de struct

    //Permet de convertir le fichier texte en un reader utilisable pour le reste du programme.

    reader = xmlNewTextReaderFilename( cellFile.c_str() ); //si le reader est NULL c'est que l'on a pas réussi à ouvrir le fichier
    
    if (reader == NULL) {
      cerr << "[ERROR] Cell::load() unable to open file <" << cellFile << ">." << endl;
      return NULL;
    }
    //on va convertir le reader en une structure de données
    Cell* cell = Cell::fromXml( reader );
    //on libère le pointeur reader.
